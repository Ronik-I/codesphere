...
router.post("/createProject", async (req, res) => {
  let { userId, title , type} = req.body;
  let user = await userModel.findOne({ _id: userId });
  if (user) {
...
router.post("/updateProject", async (req, res) => {
  let { userId, htmlCode, cssCode, jsCode, projId,code, input, language } = req.body;
  let user = await userModel.findOne({ _id: userId });

  if (user) {
    let project = await projectModel.findOneAndUpdate(
      { _id: projId },
      { htmlCode: htmlCode, cssCode: cssCode, jsCode: jsCode,code: code, input: input, language: language },
